...
export class HourlyNotificationScheduler {
  ...
  private sendNotification(notification: HourlyNotification): void {
    try {
      // Normalize types: internal uses 'danger' while NotificationService expects 'error'
      const normalizedType = notification.type === 'danger' ? 'error' : notification.type;

      // Use centralized NotificationService so the popup matches the app's standard
      // flash + toast appearance (title + description + small toast message).
